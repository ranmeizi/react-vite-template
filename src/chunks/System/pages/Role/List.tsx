...
export default function () {
  ...
  const checkedKeys = useMemo(() => {
    return Object.values(map.current)
      .filter((item) => {
        if (!item.permission) {
          return true;
        } else {
  ...
  function onCheck(checkedKeys: any, { checkedNodes }: any) {
    dispatch(
      setPermission(
        checkedNodes.reduce((list: any, curr: any) => {
          if (curr.permission) {
  ...
  return (
    <Page>
      ...
      <div>
        ...
        <div>
          <Link to={"/f/homepage"}>工作台</Link>
        </div>
        <div>
          <Link to={"/f/example/list"}>示例列表页</Link>
        </div>
      </div>
    </Page>
  );
